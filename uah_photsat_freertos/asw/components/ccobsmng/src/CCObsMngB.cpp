	...
	do
	{
		...
		switch(edroomCurrentTrans.localId)
		{
			...
			case (DoAttitudeCtrl):
				...
				//Execute Action 
				FDoAttitudeCtrl();
				//Evaluate Branch FToObservation
				if( GReadyToObservation() )
				{
					//Execute Action 
					FToObservation();

					//Branch taken is DoAttitudeCtrl_FToObservation
			...
			case (TakeImage):
				...
				break;
			//Next Transition is FExecObservationTC
			case (FExecObservationTC):
				//Msg->Data Handling 
				FExecObsMng_TC();
				//Next State is Standby
				edroomNextState = Standby;
	...
	do
	{
		...
		switch(Msg->signal)
		{
			...
			case (EDROOMSignalTimeout): 
				 ...
				 if (*Msg->GetPInterface() == AttCtrltTmer)
				{
			...
			case (SObsMng_TC): 
				 ...
				 if (*Msg->GetPInterface() == ObsMng)
				{
					...
					//Next transition is  FExecObservationTC
					edroomCurrentTrans.localId= FExecObservationTC;
					edroomCurrentTrans.distanceToContext = 0;
					edroomValidMsg=true;
				 }
